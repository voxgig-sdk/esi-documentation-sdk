...
class EsiDocumentationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = EsiDocumentationControl()
        ...
        if isinstance(sp, EsiDocumentationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, EsiDocumentationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, EsiDocumentationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return EsiDocumentationOperation({})
    ...
    def make_error(self, code, msg):
        return EsiDocumentationError(code, msg, self)
