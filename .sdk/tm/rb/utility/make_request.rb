...
module EsiDocumentationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EsiDocumentationResponse.new({})
    ...
    if fetch_err
      ...
      response = EsiDocumentationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EsiDocumentationResponse.new(fetched)
    else
