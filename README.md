...
Learn more about ESI Documentation at [esi.evetech.net](https://esi.evetech.net).
...
This SDK exposes the API as a small set of **semantic entities** — Asset, Character and Structure — that you
...
const items = await client.Asset().list()
```
...
// assets is an array of bare Asset records populated with mock data
console.log(assets)
```
...
```go
client := sdk.Test()
result, err := client.Asset(nil).List(
    nil, nil,
)
```
