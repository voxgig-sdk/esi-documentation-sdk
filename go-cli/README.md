# esi-documentation-cli

AQL-driven command-line client **and** interactive REPL for the EsiDocumentation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/esi-documentation-cli)
make build

# 2. See usage (words, entities, env vars)
./esi-documentation-cli --help
...
./esi-documentation-cli list asset
./esi-documentation-cli list character

# 5. Override the API base URL for a single call
ESI_DOCUMENTATION_BASE=https://api.example.com ./esi-documentation-cli list asset

# 6. No arguments -> interactive REPL
./esi-documentation-cli
esi-documentation> list asset
esi-documentation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/esi-documentation-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/esi-documentation-cli list asset
   ```
...
Run with no arguments to open a REPL (prompt `esi-documentation>`). Each line is
evaluated as its own AQL expression:

```text
$ ./esi-documentation-cli
esi-documentation> list asset
esi-documentation> /help
esi-documentation> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/esi-documentation-cli`. |
...
(`./esi-documentation-cli <expr>`) and interactively (the REPL), and expressions compose the
