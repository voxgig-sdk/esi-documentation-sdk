// Command EsiDocumentation-cli is a CLI + REPL for the EsiDocumentation SDK,
...
const prompt = "esi-documentation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "asset character structure"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ESI_DOCUMENTATION_APIKEY (api key), ESI_DOCUMENTATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ESI_DOCUMENTATION_APIKEY carries the API key and
	// ESI_DOCUMENTATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ESI_DOCUMENTATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ESI_DOCUMENTATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEsiDocumentationSDK(opts)
