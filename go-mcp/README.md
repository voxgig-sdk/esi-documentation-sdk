...
[MCP](https://modelcontextprotocol.io) server exposing the EsiDocumentation SDK as
two agent tools — `esi-documentation_list` and `esi-documentation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/esi-documentation-mcp)
make build

# 2. Provide credentials via the environment
export ESI_DOCUMENTATION_APIKEY=sk_live_xxx
...
```jsonc
// esi-documentation_list: first page of records
{ "entity": "asset" }
{ "entity": "asset", "query": { } }

// esi-documentation_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/esi-documentation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user esi-documentation \
     -- "$PWD"/dist/*/esi-documentation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `esi-documentation_list` and `esi-documentation_load` tools now appear
   in new sessions. Ask the agent to *"list asset using esi-documentation"*
   and it calls `esi-documentation_list` with `{"entity":"asset"}`.
...
```sh
export ESI_DOCUMENTATION_APIKEY=sk_live_xxx            # API key
export ESI_DOCUMENTATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `esi-documentation_list` tool
...
### Call the `esi-documentation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `esi-documentation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `esi-documentation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ESI_DOCUMENTATION_APIKEY` | API key sent with every request. |
| `ESI_DOCUMENTATION_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"esi-documentation_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
