...
func TestAssetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ESIDOCUMENTATION_TEST_ASSET_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		_, assetRef01ListOk := assetRef01ListResult.([]any)
		if !assetRef01ListOk {
...
func assetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"asset01", "asset02", "asset03", "character01", "character02", "character03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("ESIDOCUMENTATION_TEST_ASSET_ENTID")
