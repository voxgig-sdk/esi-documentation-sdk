# EsiDocumentation Lua SDK

The Lua SDK for the EsiDocumentation API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Asset(nil):list(nil, nil)
...
local result, err = client:EsiDocumentation(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── esi-documentation_sdk.lua    -- Main SDK module
...
The main module (`esi-documentation_sdk`) exports the SDK constructor
