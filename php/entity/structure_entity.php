...
// EsiDocumentation SDK Structure entity
...
class StructureEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new StructureEntity($this->_client, $opts);
    }

    /**
     * @param Structure|array $args Structure data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EsiDocumentationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Structure|array The current Structure data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Structure fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EsiDocumentationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Structure fields).
     */
     ...
     *
     * @param StructureLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed StructureLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Structure|array The loaded Structure as an assoc-array at the
     *   SDK boundary; throws EsiDocumentationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EsiDocumentationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
