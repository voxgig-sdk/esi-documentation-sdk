...
// EsiDocumentation SDK retry feature
...
class EsiDocumentationRetryFeature extends EsiDocumentationBaseFeature
{
    ...
    public function init(EsiDocumentationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EsiDocumentationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EsiDocumentationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EsiDocumentationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
