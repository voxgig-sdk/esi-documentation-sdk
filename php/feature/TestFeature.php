...
// EsiDocumentation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class EsiDocumentationTestFeature extends EsiDocumentationBaseFeature
{
    ...
    public function init(EsiDocumentationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (EsiDocumentationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = EsiDocumentationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (EsiDocumentationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(EsiDocumentationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = EsiDocumentationParam::call($ctx, $k);
