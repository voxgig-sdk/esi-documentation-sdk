...
// EsiDocumentation SDK feature test
//
...
class FtCtrl extends EsiDocumentationControl
{
...
class FtClient
{
    ...
    private EsiDocumentationUtility $utility;

    public function __construct(EsiDocumentationUtility $utility, array $options)
    {
    ...
    public function get_utility(): EsiDocumentationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EsiDocumentationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EsiDocumentationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EsiDocumentationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EsiDocumentationConfig::make_feature($name);
    ...
    private static function buildUrl(EsiDocumentationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EsiDocumentationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EsiDocumentationResponse($fetched);
        }
    ...
    private function fail(EsiDocumentationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EsiDocumentationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
