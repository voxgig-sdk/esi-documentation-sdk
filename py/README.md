# EsiDocumentation Python SDK

The Python SDK for the EsiDocumentation API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Asset(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.EsiDocumentation(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── esidocumentation_sdk.py         -- Main SDK module
