...
from esidocumentation_types import (
    Structure,
    StructureLoadMatch,
)
...
class StructureEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StructureEntity(self._client, opts)
    ...
    def match_get(self) -> Structure:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> Structure:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Structure().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
