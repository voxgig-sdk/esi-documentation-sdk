...
class TestStructureDirect:
    ...
    def test_should_direct_load_structure(self):
        setup = _structure_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-structure", "live" if setup["live"] else "unit")
        if _skip:
...
def _structure_direct_setup(mockres):
