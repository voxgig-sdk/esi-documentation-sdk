...
class StructureEntity
  ...
  def make
    opts = @_entopts.dup
    StructureEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = EsiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Structure, Hash] the current Structure data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = EsiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Structure fields)
  def match_get
  ...
  #
  # @param reqmatch [StructureLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Structure.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Structure, Hash] the loaded Structure; raises EsiDocumentationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EsiDocumentationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
