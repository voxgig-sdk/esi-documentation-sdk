...
class AssetDirectTest < Minitest::Test
  def test_direct_list_asset
    setup = asset_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-asset", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["character01"].each do |_live_key|
    ...
    params = {}
    if setup[:live]
      params["character_id"] = setup[:idmap]["character01"]
    else
      params["character_id"] = "direct01"
    end
