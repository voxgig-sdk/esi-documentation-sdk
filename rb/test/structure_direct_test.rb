...
class StructureDirectTest < Minitest::Test
  def test_direct_load_structure
    setup = structure_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-structure", setup[:live] ? "live" : "unit")
    if _should_skip
...
def structure_direct_setup(mockres)
