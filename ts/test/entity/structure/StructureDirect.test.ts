...
describe('StructureDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new EsiDocumentationSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-load-structure', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-structure', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ESIDOCUMENTATION_TEST_STRUCTURE_ENTID']
