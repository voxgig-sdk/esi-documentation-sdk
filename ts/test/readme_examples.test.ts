...
//     (\`new EsiDocumentationSDK(...)\` / \`EsiDocumentationSDK.test(...)\` become
//     \`EsiDocumentationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EsiDocumentationSDK'
...
const TEST_SEED = {"entity":{"asset":{"test01":{"id":"test01"}},"character":{"test01":{"id":"test01"}},"structure":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EsiDocumentationSDK(...)\` and any
// \`EsiDocumentationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EsiDocumentationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EsiDocumentationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
